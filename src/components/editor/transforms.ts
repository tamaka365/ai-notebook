...
export const insertBlock = (
  ...
  editor.tf.withoutNormalizing(() => {
    ...
    const blockHandler = (insertBlockMap as Record<string, (editor: PlateEditor, type: string) => void>)[type];
    if (blockHandler) {
      blockHandler(editor, type);
    } else {
...
export const setBlockType = (
  ...
  editor.tf.withoutNormalizing(() => {
    const setEntry = (entry: NodeEntry<TElement>) => {
      ...
      const setBlockHandler = (setBlockMap as Record<string, (editor: PlateEditor, type: string, entry: NodeEntry<TElement>) => void>)[type];
      if (setBlockHandler) {
        return setBlockHandler(editor, type, entry);
      }
      if (node.type !== type) {
